...
-- employee_id 가 103번인 사람의 job_id랑 동일한 job_id를 가진 사람을 조회.
...
-- 250 -> false, 200보다는 크지만 300,400 보다는 크지 않으므로.
SELECT 
    *
FROM employees
...
JOIN (SELECT 
    ...
    FROM employees
    GROUP BY department_id) tbl
ON e.department_id = tbl.department_id AND e.salary = tbl.max_salary;
